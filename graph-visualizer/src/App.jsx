...
function makeExpansionKey(nodeId, direction) {
...
function parseExpansionKey(key) {
  ...
  return {
    nodeId: key.slice(0, index),
    direction: key.slice(index + 2),
  };
}
...
function computeFocusState(targetId, expandedKeys, prereqMap, successorMap, byId) {
  ...
  for (const id of getDirectPrereqs(targetId, prereqMap, byId)) {
    addFocusNode({
      id,
      level: -1,
      parent: { sourceId: targetId, direction: 'prereq' },
      visibleIds,
      levels,
      parents,
    });
  ...
  for (const id of getDirectSuccessors(targetId, successorMap, byId)) {
    addFocusNode({
      id,
      level: 1,
      parent: { sourceId: targetId, direction: 'successor' },
      visibleIds,
      levels,
      parents,
    });
  ...
  // Each expanded key reveals only the clicked node's direct neighbors in the allowed direction.
  // A newly revealed node never expands automatically unless that node itself has an expanded key.
  let changed = true;
  let guard = 0;
  while (changed && guard < 50) {
    ...
    for (const key of expandedKeys) {
      ...
      for (const neighbor of neighbors) {
        const added = addFocusNode({
          id: neighbor,
          level: nextLevel,
          parent: { sourceId: parsed.nodeId, direction: parsed.direction },
          visibleIds,
          levels,
          parents,
        });
...
function distributeByLevel(ids, levels, byId) {
  ...
  for (const [level, levelIds] of buckets.entries()) {
    ...
    levelIds.forEach((id, index) => {
      const y = (index - (total - 1) / 2) * Y_GAP;
      positions.set(id, { x: level * X_GAP, y });
    });
  }
  return positions;
}
...
function buildStableFocusLayout({
  targetId,
  visibleIds,
  levels,
  parents,
  ...
  if (!previousPositions.size || !previousVisibleIds.has(targetId)) {
    ...
    for (const id of visibleIds) {
      ...
      initial.set(id, {
        x: level * X_GAP,
        y: id === targetId ? 0 : sourcePosition.y,
      });
    }
  ...
  for (const id of visibleIds) {
    ...
    const level = levels.get(id) ?? 0;
    if (oldPosition) {
      next.set(id, { x: level * X_GAP, y: id === targetId ? 0 : oldPosition.y });
    }
  }
  ...
  for (const group of groups.values()) {
    ...
    group.ids.forEach((id, index) => {
      const level = levels.get(id) ?? 0;
      const offset = (index - (total - 1) / 2) * Y_GAP;
      next.set(id, {
        x: level * X_GAP,
        y: (sourcePosition?.y ?? 0) + offset,
      });
    });
  }
  ...
  for (const id of visibleIds) {
    if (!next.has(id)) {
      const level = levels.get(id) ?? 0;
      next.set(id, { x: level * X_GAP, y: id === targetId ? 0 : 0 });
    }
  }
...
function ConceptNode({ data }) {
  ...
  return (
    <div className="concept-node-wrap" style={{ width: size, height: size }}>
      ...
      {showLeftExpand && (
        <button
          className="expand-chip expand-chip-left nodrag nopan"
          title="직접 선행 개념 펼치기"
          onClick={(event) => {
            event.stopPropagation();
            data.onToggleExpand(data.id, 'prereq');
          }}
        >
      ...
      {canUseLeftControls && data.isPrereqExpanded && !showLeftExpand && (
        <button
          className="collapse-chip collapse-chip-left nodrag nopan"
          title="이 노드에서 펼친 직접 선행 개념 접기"
          onClick={(event) => {
            event.stopPropagation();
            data.onToggleExpand(data.id, 'prereq');
          }}
      ...
      {showRightExpand && (
        <button
          className="expand-chip expand-chip-right nodrag nopan"
          title="직접 후속 개념 펼치기"
          onClick={(event) => {
            event.stopPropagation();
            data.onToggleExpand(data.id, 'successor');
          }}
        >
      ...
      {canUseRightControls && data.isSuccessorExpanded && !showRightExpand && (
        <button
          className="collapse-chip collapse-chip-right nodrag nopan"
          title="이 노드에서 펼친 직접 후속 개념 접기"
          onClick={(event) => {
            event.stopPropagation();
            data.onToggleExpand(data.id, 'successor');
          }}
      ...
      <div
        className={`concept-node ${data.isTarget ? 'target-node' : ''} ${data.isSelected ? 'selected-node' : ''}`}
        style={{
          width: size,
          height: size,
  ...
  for (const concept of concepts) {
    ...
    for (const prereq of prereqMap.get(concept.id) || []) {
      ...
      edges.push({
        ...
        style: {
          stroke: touchesTarget ? '#334155' : '#9CA3AF',
          strokeWidth: touchesTarget ? 2.4 : 1.6,
        },
      });
    }
  }
  ...
  return Array.from(visibleIds).map((id) => {
    ...
    return {
      ...
      data: {
        ...
        isPrereqExpanded: expandedKeys.has(makeExpansionKey(id, 'prereq')),
        isSuccessorExpanded: expandedKeys.has(makeExpansionKey(id, 'successor')),
        onToggleExpand,
      },
    };
  });
...
export default function App() {
  ...
  const [targetId, setTargetId] = useState('matrix_rank');
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setStudentState(stateJson || {});
        if (!conceptList.some((concept) => concept.id === targetId) && conceptList.length > 0) {
  ...
  const contexts = useMemo(() => {
    const set = new Set();
    for (const concept of concepts) {
      for (const context of concept.learning_contexts || []) set.add(context);
    }
    ...
    return concepts
      .filter((concept) => {
        ...
        const matchesSearch =
          ...
          (concept.name_kr || '').toLowerCase().includes(query) ||
          (concept.name_en || '').toLowerCase().includes(query);
  ...
  useEffect(() => {
    if (targetOptions.length > 0 && !targetOptions.some((concept) => concept.id === targetId)) {
  ...
  const handleToggleExpand = useCallback((nodeId, direction) => {
    setExpandedKeys((prev) => {
      ...
      const mode = next.has(key) ? 'collapse' : 'expand';
      if (next.has(key)) next.delete(key);
      else next.add(key);
      lastActionRef.current = { nodeId, direction, mode };
      return next;
    });
  }, []);
  ...
  const handleConceptUpload = useCallback(async (event) => {
    ...
    try {
      ...
      setLoadError(`개념 JSON 업로드 실패: ${error.message}`);
    } finally {
      event.target.value = '';
    }
  }, []);
  ...
  const handleStudentStateUpload = useCallback(async (event) => {
    ...
    try {
      const json = await readJsonFile(file);
      setStudentState(json || {});
      setLoadError('');
    } catch (error) {
      setLoadError(`학생 상태 JSON 업로드 실패: ${error.message}`);
    } finally {
      event.target.value = '';
    }
  }, []);
  ...
  const focusState = useMemo(() => {
    if (!concepts.length || viewMode !== 'focus') {
      return { visibleIds: new Set(), levels: new Map(), parents: new Map() };
    }
  ...
  const visibleIds = useMemo(() => {
    ...
    if (viewMode === 'full') {
      return new Set(
        concepts
          .filter((concept) => selectedArea === ALL_AREAS || (concept.primary_area || 'other') === selectedArea)
  ...
  useEffect(() => {
    setExpandedKeys((prev) => {
      ...
      for (const key of prev) {
        const parsed = parseExpansionKey(key);
        if (parsed && visibleIds.has(parsed.nodeId)) next.add(key);
      }
  ...
  useEffect(() => {
    ...
    if (viewMode === 'full') {
      ...
      setPositionMap(fullPositions);
      previousVisibleIdsRef.current = new Set(visibleIds);
      previousPositionsRef.current = fullPositions;
      return;
    }

    const previousVisibleIds = previousVisibleIdsRef.current;
    const previousPositions = previousPositionsRef.current;
    const nextPositions = buildStableFocusLayout({
      targetId,
      visibleIds,
      levels: focusState.levels,
      parents: focusState.parents,
      previousVisibleIds,
      previousPositions,
      byId,
    });

    setPositionMap(nextPositions);
    previousVisibleIdsRef.current = new Set(visibleIds);
  ...
  useEffect(() => {
    ...
    // Deliberately do not depend on nodes.length. Expansion should not pan/zoom the canvas.
      ...
      <aside className="sidebar">
        ...
        {viewMode === 'focus' ? (
          <>
            <section className="panel-section">
              ...
              <p className="field-help">context는 target 후보를 좁히는 필터입니다. 펼치기는 실제 prerequisite 관계를 따릅니다.</p>
            </section>
            ...
            <section className="panel-section">
              ...
              <input
                ...
                placeholder="예: 랭크, rank, matrix_rank"
              />
            </section>
            ...
            <section className="panel-section">
              ...
              >
                {targetOptions.map((concept) => (
                  <option key={concept.id} value={concept.id}>{getConceptLabel(concept)} · {concept.id}</option>
                ))}
              </select>
        ...
        <section className="panel-section">
          ...
          <label className="file-button">
            개념 JSON 교체
            <input type="file" accept="application/json,.json" onChange={handleConceptUpload} />
          </label>
          <label className="file-button">
            학생 상태 JSON 교체
            <input type="file" accept="application/json,.json" onChange={handleStudentStateUpload} />
          </label>
        </section>
        ...
        {selectedConcept && (
          <section className="detail-card">
            ...
            <div className="detail-meta">
              <span>{AREA_LABELS[selectedConcept.primary_area] || selectedConcept.primary_area || '영역 없음'}</span>
            </div>
            <div className="detail-contexts">
              {(selectedConcept.learning_contexts || []).map((context) => (
            ...
            <div className="detail-block">
              ...
              <ul>
                {(selectedConcept.must_have || []).slice(0, 3).map((item) => <li key={item}>{item}</li>)}
              </ul>
            </div>
            <div className="detail-block">
      ...
      <main className="flow-area">
        <div className="top-bar">
          ...
          <div className="rule-summary">
            {viewMode === 'focus'
              ? '초기: target + 직접 선행 + 직접 후속 · 선행은 왼쪽으로만, 후속은 오른쪽으로만 1단계씩 펼침'
              : '전체 그래프: primary_area 필터와 prerequisite DAG 표시'}
          </div>
        </div>
        ...
        >
          ...
          <MiniMap
            ...
            zoomable
            nodeColor={(node) => {
              const level = clampMastery(node.data?.masteryLevel ?? 0);
              return MASTERY[level].color;
            }}
          />
        </ReactFlow>
      </main>
